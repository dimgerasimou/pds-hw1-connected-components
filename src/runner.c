 ...
 */
static int run_benchmark(const char *binary, const char *matrix_file,
    ...
    if (pid == 0) {
        ...
        execl(binary, binary, matrix_file, "-t", threads_str, "-n", trials_str, NULL);
        perror("execl");
        exit(1);
    }
 ...
 */
static char* extract_json_block(const char *json, const char *key, char open, char close) {
    char search[128];
 ...
 */
static char* extract_single_result(const char *results_array) {
 ...
 */
static char* extract_results_array(const char *json_output) {
 ...
 */
static void print_combined_results(BenchmarkResult *results, int count) {
    char *sys_info = NULL;
    char *matrix_info = NULL;
    char *benchmark_info = NULL;

    // Extract sys_info, matrix_info, and benchmark_info from first successful result
    ...
    if (!sys_info || !matrix_info || !benchmark_info) {
        fprintf(stderr, "Error: missing sections in benchmark JSON.\n");
        free(sys_info);
        free(matrix_info);
        free(benchmark_info);
        return;
    ...
    printf("{\n");
    printf("  \"sys_info\": %s,\n", sys_info);
    printf("  \"matrix_info\": %s,\n", matrix_info);
    printf("  \"benchmark_info\": %s,\n", benchmark_info);
    printf("  \"results\": [\n");
...
static void print_usage(const char *prog_name) {
    fprintf(stderr, "Usage: %s <matrix_file> <threads> <trials>\n", prog_name);
    fprintf(stderr, "\n");
    fprintf(stderr, "Arguments:\n");
    fprintf(stderr, "  matrix_file  Path to the input matrix file\n");
    fprintf(stderr, "  threads      Number of threads to use\n");
    fprintf(stderr, "  trials       Number of benchmark trials\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "Example:\n");
    fprintf(stderr, "  %s data/matrix.mat 4 10\n", prog_name);
}
...
int main(int argc, char *argv[]) {
    ...
    if (threads <= 0 || trials <= 0) {
        fprintf(stderr, "Error: threads and trials must be positive integers\n");
        return 1;
    }

    if (access(matrix_file, R_OK) != 0) {
        fprintf(stderr, "Error: cannot access matrix file '%s': %s\n",
